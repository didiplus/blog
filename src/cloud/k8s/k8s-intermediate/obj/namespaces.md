...
- [3.2.4、名称空间](#324名称空间)
  ...
  - [如何使用名称空间](#如何使用名称空间)
    - [查看名称空间](#查看名称空间)
    - [在执行请求时设定namespace](#在执行请求时设定namespace)
    - [设置名称空间偏好](#设置名称空间偏好)
  - [名称空间与DNS](#名称空间与dns)
...
名称空间为 [名称](./names.md) 提供了作用域。名称空间内部的同类型对象不能重名，但是跨名称空间可以有同名同类型对象。名称空间不可以嵌套，任何一个Kubernetes对象只能在一个名称空间中。
...
大部分的 Kubernetes 对象（例如，Pod、Service、Deployment、StatefulSet等）都必须在名称空间里。但是某些更低层级的对象，是不在任何名称空间中的，例如 [nodes](../architecture/nodes.md)、[persistentVolumes]()、[storageClass]() 等
