...
一个Kubernetes对象代表着用户的一个意图（a record of intent），一旦您创建了一个Kubernetes对象，Kubernetes将持续工作，以尽量实现此用户的意图。创建一个 Kubernetes对象，就是告诉Kubernetes，您需要的集群中的工作负载是什么（集群的 **目标状态**）。请参考 [控制器](../architecture/controller.md)、[声明式配置](/learning/k8s-intermediate/workload/wl-deployment/#deployment-概述)
...
Kubernetes通过对应的[控制器](../architecture/controller.md)，不断地使实际状态趋向于您期望的目标状态。
...
```yaml
@include(../../../../.vuepress/public/statics/learning/obj/deployment.yaml)
```
